...
- **Version:** `2607.21.0` (`YYMM.major.patch`). Kept in sync between
...
- **A TV sweep is streamed, never accumulated.** Episodes are read a page at a
  time and written every `EPISODE_BATCH`. Holding a library of them before the
  ...
  percentage, with no exception anywhere to explain it.
