...
- **Version:** `2607.63.0` (`YYMM.major.patch`). Kept in sync between
...
- **Statement counts on SQLite lie about inserts.** SQLAlchemy batches an ORM
  insert into one statement only where we supply the primary key; an autoincrement
  id forces one INSERT per row locally, and batches on Postgres. `movies` and
  `plex_copies` batch; `ratings`, `seasons` and `file_jobs` do not. Count reads and
  updates, which mean the same thing in both dialects, and never "fix" a per-row
  insert without checking which side of that line the table falls on.
...
- **No synchronous database work inside an `async` endpoint.** On SQLite a session
  call is microseconds and invisible; against a hosted database it is a round trip
  that blocks the entire process while it runs. `PosterCache` was the acute case —
  thirty concurrent thumbnails, each blocking the loop — and now goes through
  `db.session.in_thread`. **Twelve endpoints still do it**: `canon_refresh`,
  `missing_recommendations`, `ask`, `save_config`, `save_actions`, `set_sections`,
  `request_canon_batch`, both `start_scan`s, settings `get_all`, `show_suggestions`,
  `version_status`, `update`. Each is one request rather than thirty, so none is
  urgent, but the fix is the same wherever it next hurts.
- **A credential in a query string must be the short-lived one.** Posters,
  downloads and the scan socket cannot send headers, so they carry a token in the
  URL, where proxies and history keep it. The auth gate *waits* for the asset token
  before rendering — kicking the mint off without waiting made the session token the
  ordinary occupant of that slot — and the mint is timed out so a hang cannot hold
  the app behind it.
