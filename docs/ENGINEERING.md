...
- **Version:** `2607.66.0` (`YYMM.major.patch`). Kept in sync between
...
- **No synchronous database work inside an `async` endpoint.** On SQLite a session
  ...
  that blocks the entire process while it runs. FastAPI runs a plain `def` handler
  in a threadpool, so only `async def` handlers are exposed. `PosterCache` was the
  acute case — thirty concurrent thumbnails — and the Settings, TV-suggestions and
  recommendations reads followed. The rest are enumerated in
  `test_event_loop_hygiene.py`, which fails on a new one **and** on a stale entry;
  do not keep that list here as well, or the two will disagree.
